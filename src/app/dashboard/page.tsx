...
async function getFileUrl(doc: Document): Promise<string | undefined> {
    if (doc.fileId) {
        ...
        if (file) {
            // Use createObjectURL for efficiency with large files
            return URL.createObjectURL(file);
        }
    }
    // Fallback for legacy data that might have fileUrl
    return doc.fileUrl;
}
    ...
    if (file) {
       try {
            ...
            if (newWindow) {
                ...
                // For excel, read and parse it
                if (isExcel) {
                    const htmlContent = await renderExcelAsHtml(file);
                    newWindow.document.write(htmlContent);
                    newWindow.document.close();
                } else {
                    // For other types, use createObjectURL for efficiency
                    ...
                    // but we can't do it immediately because the iframe needs it. 
                    // The browser will automatically revoke it when the new window/tab is closed.
                }
  ...
  const handleDownload = async (doc: Document) => {
    ...
    if (url && doc.fileName) {
      try {
        ...
        // For object URLs, it's good practice to revoke them after use.
